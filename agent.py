...
# 完整的 index.html 前端樣板 (頂部包含志賀高原即時氣溫與降雪預報模組、雪道圖直連、東京深夜交通指引與多樣化美食推薦)
...
<body class="bg-slate-950 text-slate-100 font-sans min-h-screen">
  ...
  <section class="max-w-4xl mx-auto px-4 mt-6">
    <div id="weather-card" class="bg-slate-900/90 rounded-2xl border border-sky-500/30 p-5 shadow-2xl backdrop-blur">
      <div class="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
        ...
        <span class="text-[10px] md:text-xs bg-sky-950 text-sky-300 px-2.5 py-1 rounded-full border border-sky-800 font-mono flex items-center gap-1">
  ...
  <script>
          ...
          if (day.ski_resort) {
            ...
            skiHtml = '<div class="bg-slate-950/80 p-4 rounded-xl border border-sky-500/30 space-y-3">' +
              '<div class="flex items-center justify-between flex-wrap gap-2">' +
                ...
                '</h4>' +
                '<a href="' + (day.ski_resort.official_link || "#") + '" target="_blank" rel="noopener noreferrer" class="text-xs bg-sky-600 hover:bg-sky-500 text-white px-2.5 py-1 rounded transition flex items-center gap-1">' +
    ...
    async function fetchShigaWeather() {
      ...
      try {
        ...
        var currentHumidity = current.relative_humidity_2m;
